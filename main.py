...
async def about(ctx):
    embed = discord.Embed(
        ...
        description="SefiBOT, developed by **BleizeN**",
        timestamp=datetime.utcnow(),
        color=discord.Color.blue(),
    )
    embed.add_field(
        name="Server created by testing proposals at", value=f"{ctx.guild.created_at}"
    )
    embed.add_field(name="Server owner", value=f"{ctx.guild.owner}")
    embed.add_field(name="Server region", value=f"{ctx.guild.region}")
    embed.add_field(name="Server ID", value=f"{ctx.guild.id}")
    embed.set_thumbnail(
        url="https://pbs.twimg.com/profile_images/1401793142324699136/ECowi4TP_400x400.jpg"
    )
